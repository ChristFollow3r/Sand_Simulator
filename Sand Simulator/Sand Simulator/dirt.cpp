...
void Dirt::ApplyPhysics(Block* grid, std::vector<std::shared_ptr<Material>>& additions, float dt) {
	...
	if (grid[gridIndex + cols].type == AirType) moveTo(gridIndex + cols);
		...
		else if (leftfree) { moveTo(this->gridIndex + cols - 1); return; }
		else if (rightfree) { moveTo(this->gridIndex + cols + 1); return; }
